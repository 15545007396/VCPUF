...
def svm(input_bit, x1, y1):
    last_acc = []
    crpnum = []
    for for_train in range(5,14):#×îÐ¡µÄtrain_sizeÊÇ2**10µÄ1/2**7
        train_size = 1/(2**for_train)
        test_size = 1 - train_size
...
np.savez("\\npzdata\\iPUF-4+16-20230319.npz", FV=FV, RES=RES)


# In[ ]:


D = np.load("\\npzdata\\iPUF-4+16-20230319.npz")
FV = D['FV']
RES = D['RES']
...
totaldf.to_csv('\\iPUF-4+16-20230319.csv')
