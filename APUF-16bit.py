...
int_cha = random.sample(range(0,2**input_bit),2**gen)#sampleÆäÊµÊÇ²ÉÑù£¬ÎªÁË±ÜÃâÖØ¸´£¬ÔÚÁÐ±íÖÐ½øÐÐ²ÉÑù
...
for i in int_cha:
    ...
    lfv.append(fv)
    #Éú³ÉÒì»òÇ°µÄÑÓÊ±Öµ
    delay = np.sum([a*b for a,b in zip(fv,stage[:,0])])#from ·É
...
np.savez('APUF'+str(input_bit)+str(datetime.date.today())+'.npz', FV=FV, RES=RES)


# In[8]:


D = np.load('APUF'+str(input_bit)+str(datetime.date.today())+'.npz')
FV = D['FV']
RES = D['RES']
