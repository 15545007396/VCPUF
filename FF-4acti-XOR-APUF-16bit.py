...
def ANN(input_bit, FV, RES):
    ...
    weights = {
        'delay':tf.Variable(tf.random_normal([input_bit,1])),
    }
...
for i in int_cha:
    ...
    lfv.append(fv)
    ################################ First chain ##############################
    arb3_1 = np.sum([a*b for a,b in zip(fv[0:input_bit//3],stage_1[0:input_bit//3])])
    ...
    ################################ Second chain #############################
    arb3_2 = np.sum([a*b for a,b in zip(fv[0:input_bit//3],stage_2[0:input_bit//3])])
...
np.savez('FF4XORAPUF-16bit.npz', FV=FV, RES=RES)


# In[7]:


D = np.load("FF4XORAPUF-16bit.npz'")
FV = D['FV']
RES = D['RES']
...
totaldf.to_csv('FF4XORAPUF-16bit.csv')
